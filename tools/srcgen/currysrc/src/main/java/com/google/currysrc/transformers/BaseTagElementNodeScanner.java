...
public abstract class BaseTagElementNodeScanner extends BaseJavadocNodeScanner {
  ...
  @Override protected final void visit(Javadoc javadoc, final ASTRewrite rewrite) {
    javadoc.accept(new ASTVisitor(true /* visitDocTags */) {
      @Override public boolean visit(TagElement node) {
        return visitTagElement(rewrite, node);
      }
    });
  }

  protected abstract boolean visitTagElement(ASTRewrite rewrite, TagElement tag);
}
